...
public class apply extends JFrame{
	class judgeThread extends Thread
	{
		...
		public void run()
		{
			try
			{
				Thread.sleep(120000);  //睡眠120秒，何解？
				//等待 120 秒后，还是无法和服务器接通，无法得到服务器的回复
				if(t.changed == 0) // 为开始状态 刚刚创建状态
				{
					t.stop();  //停止线程，不安全方法
					changed = 1;   //  这个changed是属于judgeThread的changed，不是t（applicateClientThread）的changed
								// 因此这个是1 代表不成功；
					this.stop();  //这个apply申请线程停止
				}
			}
	...
	private JCheckBox		jc; // 这个是何解？ checkBox?
	private CardLayout		cardl;  //  卡片布局
	...
	private void jb_sureactionPerformed(ActionEvent e)
	{
		...
		else
		{
			...
			try
			{
				...
				judge.start();
				appliate.join(15000);//其他线程等待这个线程15秒，15秒内这个线程没死，其他线程继续执行
				if(appliate.changed == 1) // 服务器响应，就是已经申请到的状态
				{
					QQ = appliate.getQQ;
					jl3_4.setText(QQ);
					jl3_5.setText("你的密码是：");
				...
				else if(appliate.changed == 4)  // 4是抛出错误，提示服务器没开启也没问题
				{
